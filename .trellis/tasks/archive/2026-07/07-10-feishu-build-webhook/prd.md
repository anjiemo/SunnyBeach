...
| 卡片 header 标题 | `{BUILD_ID} - {状态文案}`，如 `123 - 检查开始🚀`、`45 - 打包失败❌` |
| 按钮 | CI（check）：开始/成功/失败均「工作流」+「Gradle 日志」；打包（build）：开始仅「工作流」，成功/失败再加「Gradle 日志」 |
...
- 打包 id、状态、`{app_name}/{类型}`、触发方式、分支、版本、git 短哈希、git 提交首行、工作流链接、Gradle 日志链接（CI 各状态；打包仅成功/失败）、创建时间、更新时间

## 验收标准

- [ ] 手动三种 build_type 收到开始 + 终态卡片；仅 release 上传 Artifact
- [ ] Tag `v*` 正式版 + Release；触发方式为「自动触发」
- [ ] CI 使用检查开始/成功/失败；三种状态均可点工作流与 Gradle 日志（开始卡片也需真实 JOB_URL）
