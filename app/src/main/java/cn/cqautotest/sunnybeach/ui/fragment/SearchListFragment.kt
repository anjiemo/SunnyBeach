...
class SearchListFragment : TitleBarFragment<AppActivity>(), StatusAction, OnBack2TopListener {
    ...
    private val loadStateListener = loadStateListener(mSearchResultListAdapter) {
    ...
    override fun initData() {
        loadSearchResultList(DEFAULT_KEYWORDS)
    }
    ...
    private fun loadSearchResultList(keywords: String) {
        if (keywords.isEmpty()) {
            viewLifecycleOwner.lifecycleScope.launchWhenCreated {
                mSearchResultListAdapter.submitData(PagingData.empty())
                showEmpty()
            }
            return
        }
    ...
    override fun initEvent() {
        mBinding.refreshLayout.setOnRefreshListener {
            mSearchResultListAdapter.refresh()
        }
        ...
        mAdapterDelegate.setOnItemClickListener { _, position ->
            ...
            val url = when (SearchType.valueOfType(item.type)) {
                ...
                SearchType.SHARE -> "$SUNNY_BEACH_SHARE_URL_PRE${item.id}"
                else -> null
            }
            url?.let {
